...
def part_1(image):
    ...
    color_range["blue"] = [(0, 0, 0), (255, 70, 70)]
    color_range["green"] = [(0, 0, 0), (100,255,100)]
    color_range["red"] = [(0,0,0), (70, 70, 255)]
    ...
    #<YOUR CODE GOES HERE>
    blue_thresh = cv2.inRange(image, *color_range["blue"])
    green_thresh = cv2.inRange(image, *color_range["green"])
    red_thresh = cv2.inRange(image, *color_range["red"])    

    color_amount["blue"] = cv2.countNonZero(blue_thresh)
    color_amount["green"] = cv2.countNonZero(green_thresh)
    color_amount["red"] = cv2.countNonZero(red_thresh)

    print("Color amount:", color_amount)
...
def part_2(image, image_HSV):
    ...
    #<YOUR CODE GOES HERE>
    enhanced_HSV = image_HSV #Don't change the input images, store new enhanced image here
    sat_const = 100
    (H, S, V) = cv2.split(enhanced_HSV)
    S = S + sat_const
    enhanced_HSV = cv2.merge([H, S, V])
    enhanced_image = cv2.cvtColor(enhanced_HSV.astype("uint8"), cv2.COLOR_HSV2BGR)
    return enhanced_image    
    
#Main code that is being run
def color_id(image_file = 'test.jpg', show = False):
    folder_path = '/home/pi/Labs/Imaging/Images/EvelynLi' #Replace with the folder path for the folder in the
